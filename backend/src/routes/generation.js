...
// ── GET roles preview (list of roles + user counts) ───────────────────────────
...
router.post('/:projectId/generate/bulk', authenticate, requireMember(), previewLimiter, async (req, res) => {
  ...
  try {
    ...
    const appName   = params.app_name   || project.application_name || '';
    const plantName = params.plant_name || project.plant_name       || '';
    const goLive    = params.go_live_date || project.go_live_date   || '';
    ...
    for (const [roleName, group] of roleMap) {
      ...
      for (let wi = 0; wi < waves.length; wi++) {
        const waveNum = wi + 1;
        if (!params.parts_to_generate.includes(waveNum)) continue;
        ...
        const roadmap = buildDynamicRoadmap(waveNum, params.total_parts);
        let body = ROADMAP_RE.test(template.html_content)
...
// ── Legacy single-user preview (kept for compatibility) ───────────────────────
