...
export default function DashboardPage() {
  ...
  const createProject = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(['projects']); setShowCreate(false); setForm({ project_name: '', plant_name: '', application_name: '', go_live_date: '' }); }
  });

  const statusColor = { draft: 'bg-gray-100 text-gray-600', setup_in_progress: 'bg-yellow-100 text-yellow-700', ready: 'bg-green-100 text-green-700', archived: 'bg-red-100 text-red-600' };
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="grid gap-4">
          {projects.map(p => (
            <div key={p.id} onClick={() => navigate(`/projects/${p.id}`)} className="bg-white border rounded-xl p-5 cursor-pointer hover:shadow-md transition">
              <div className="flex items-start justify-between">
                ...
                <span className={`text-xs px-2 py-1 rounded-full font-medium ${statusColor[p.status] || 'bg-gray-100 text-gray-600'}`}>{p.status}</span>
              </div>
              {p.go_live_date && <p className="text-xs text-slate-400 mt-2">Go-live: {new Date(p.go_live_date).toLocaleDateString()}</p>}
            </div>
          ))}
        </div>
      </main>
