...
export default function UserListPage() {
  ...
  // Build a set of valid (non-error) function+role combos from the matrix
  const validCombos = useMemo(() => {
    const set = new Set();
    for (const e of matrixEntries) {
      if (!e.pdm_role?.toLowerCase().startsWith('error')) {
        set.add(`${e.function}||${e.role}`);
      }
    }
    return set;
  }, [matrixEntries]);
  ...
  const matrixFunctions = useMemo(() =>
    [...new Set(matrixEntries.map(e => e.function))].sort()
  , [matrixEntries]);

  // Only show roles that have at least one valid (non-error) rule for this function
  function rolesForFunction(fn) {
    return [...new Set(
      matrixEntries
        .filter(e => e.function === fn && !e.pdm_role?.toLowerCase().startsWith('error'))
        .map(e => e.role)
    )].sort();
  }

  function isInvalidCombo(fn, role) {
    if (!fn || !role) return false;
    return !validCombos.has(`${fn}||${role}`);
  }
  ...
  async function lookup(userSnapshot) {
    ...
    try {
      const res = await client.post(`/projects/${projectId}/role-matrix/lookup`, {
        function: userSnapshot.function, role: userSnapshot.role,
        pbom_champion: userSnapshot.pbom_champion, boc_admin: userSnapshot.boc_admin,
        boc_member: userSnapshot.boc_member, eto_user: userSnapshot.eto_user,
  ...
  async function handleNewUserChange(field, value) {
    ...
    // Reset role when function changes to avoid stale invalid combo
    if (field === 'function') updated.role = '';
    setNewUser(updated);
    const isLookupField = COLUMNS.find(c => c.key === field)?.lookupTrigger;
    if (isLookupField && updated.function && updated.role) {
      const result = await lookup(updated);
      if (result && result.found && !result.is_error) {
  ...
  async function commitEdit(userId, field, col, user) {
    ...
    // Reset role if function changed
  ...
  function renderCell(user, col) {
    ...
    if (col.bool) {
      return (
        <div className="flex justify-center">
          <input type="checkbox" checked={!!rawValue} onChange={() => toggleBool(user, col.key)}
    ...
    if (col.roleSelect && isEditing) {
      ...
      return (
        <select autoFocus className="border rounded px-1 py-0.5 text-xs w-full"
          ...
          {roles.length === 0 && <option value="">(no valid roles for this function)</option>}
    ...
    // Invalid combo warning badge on function/role cells
    const showInvalidBadge = col.key === 'role' && isInvalidCombo(user.function, rawValue);

    return (
      <span onClick={() => { setEditingCell(cellId); setEditValue(rawValue ?? ''); }}
        className="flex items-center gap-1 cursor-pointer hover:bg-blue-50 rounded px-1 py-0.5 truncate text-xs text-slate-700"
        title={String(rawValue ?? '')}>
        {rawValue || <span className="text-slate-300">-</span>}
        {showInvalidBadge && (
          <span className="shrink-0 text-xs bg-red-100 text-red-600 font-medium px-1 rounded" title="This Function + Role combination has no valid training rule in the Role Matrix">invalid</span>
        )}
      </span>
    );
  }
  ...
  const newUserInvalid = isInvalidCombo(newUser.function, newUser.role);
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-4 shrink-0">
        ...
        <div className="flex gap-2 items-center">
          ...
          <button onClick={() => { setShowAddForm(v => !v); setNewUser(EMPTY_USER); setAddLookupResult(null); }}
        ...
        <div className="bg-slate-50 border rounded-xl p-4 mb-4 shrink-0">
          ...
          <div className="grid grid-cols-4 gap-3 mb-3">
            {['sesa_id','first_name','last_name','mail'].map(key => (
              <div key={key}>
                ...
                <input className="border rounded-lg px-2 py-1.5 text-sm w-full" value={newUser[key]}
          ...
          <div className="grid grid-cols-3 gap-3 mb-3">
            <div>
              ...
              <select className="border rounded-lg px-2 py-1.5 text-sm w-full" value={newUser.function}
            ...
            <div>
              ...
              <select className={`border rounded-lg px-2 py-1.5 text-sm w-full ${ newUserInvalid ? 'border-red-400' : '' }`}
                value={newUser.role} onChange={e => handleNewUserChange('role', e.target.value)}
                disabled={!newUser.function}>
                <option value="">Select role...</option>
                {newUserRoles.map(r => <option key={r} value={r}>{r}</option>)}
              </select>
              {newUserInvalid && <p className="text-xs text-red-500 mt-0.5">This combination has no valid training rule</p>}
            </div>
            <div>
              <label className="text-xs text-slate-500 block mb-1">Manager Mail</label>
              <input className="border rounded-lg px-2 py-1.5 text-sm w-full" value={newUser.manager_mail}
          ...
          <div className="flex flex-wrap gap-5 mb-3 py-2 px-3 bg-white border rounded-lg">
            {COLUMNS.filter(c => c.bool).map(col => (
              <label key={col.key} className="flex items-center gap-2 text-sm cursor-pointer select-none">
                <input type="checkbox" checked={!!newUser[col.key]}
          ...
          {(newUser.recommended_training || newUser.tlg_group) && (
            <div className={`flex gap-6 mb-3 px-3 py-2 rounded-lg text-xs ${ addLookupResult?.is_error ? 'bg-red-50 text-red-600' : 'bg-blue-50 text-blue-700' }`}>
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            <div>
              ...
              <input className="border rounded-lg px-2 py-1.5 text-sm w-full" value={newUser.description}
                onChange={e => setNewUser(u => ({ ...u, description: e.target.value }))} />
            </div>
            <div>
              <label className="text-xs text-slate-500 block mb-1">Status</label>
              <select className="border rounded-lg px-2 py-1.5 text-sm w-full" value={newUser.status}
            ...
            <div>
              ...
              <input className="border rounded-lg px-2 py-1.5 text-sm w-full" value={newUser.comments}
          ...
          <div className="flex gap-2">
            <button onClick={() => createMutation.mutate(newUser)}
              ...
              className="bg-blue-600 text-white px-4 py-1.5 rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-40">Save user</button>
            <button onClick={() => { setShowAddForm(false); setNewUser(EMPTY_USER); setAddLookupResult(null); }}
      ...
      <div className="overflow-auto rounded-xl border bg-white flex-1">
        <table className="min-w-max text-sm border-collapse">
          ...
          <tbody>
            {isLoading && <tr><td colSpan={COLUMNS.length + 1} className="px-3 py-8 text-center text-slate-400">Loading...</td></tr>}
            {!isLoading && filtered.length === 0 && <tr><td colSpan={COLUMNS.length + 1} className="px-3 py-12 text-center text-slate-400">No users yet. Add one manually or import an Excel file.</td></tr>}
            {filtered.map(user => (
              <tr key={user.id} className={`border-b hover:bg-slate-50/50 transition-colors ${ lookupStatus[user.id] === 'error' ? 'bg-red-50' : '' }`}>
                {COLUMNS.map(col => (
                ...
                <td className="px-2 py-1.5">
                  <button onClick={() => deleteMutation.mutate(user.id)} className="text-red-300 hover:text-red-500 text-xs">Del</button>
                </td>
              </tr>
            ))}
          </tbody>
